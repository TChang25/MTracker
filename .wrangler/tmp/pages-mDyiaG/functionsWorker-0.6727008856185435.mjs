...
var require_TokenExpiredError = __commonJS({
  "../node_modules/jsonwebtoken/lib/TokenExpiredError.js"(exports, module) {
    ...
    init_virtual_unenv_global_polyfill_clear_immediate();
    var JsonWebTokenError = require_JsonWebTokenError();
    var TokenExpiredError2 = /* @__PURE__ */ __name(function(message, expiredAt) {
      JsonWebTokenError.call(this, message);
...
// api/global.ts
async function getJWTToken(context2) {
...
async function decodeJWTToken(context2, token) {
  const JWT_SECRET = `${context2.env.TOKEN_SECRET}`;
  try {
...
async function onRequestGet(context2) {
  const { request, env: env3 } = context2;
  console.log(`Request received: ${request.url}`);
  const token = await getJWTToken(context2);
  let JSONResponse = await responseTokenExists(token);
  if (JSONResponse) {
    return JSONResponse;
  }
  try {
    const username = await decodeJWTToken(context2, token);
    ...
    const userQuery = context2.env.DB.prepare(
    ...
    console.error("/api/allstreaks", error3);
...
async function onRequestPost(context2) {
  ...
  const userStmt = context2.env.DB.prepare("SELECT hashed_pw, salt FROM user WHERE username = ?");
  ...
  const jwt_token = await generateAccessToken(json.username, `${context2.env.TOKEN_SECRET}`);
...
var hashPassword2, generateSalt;
var init_register = __esm({
  "api/register.ts"() {
    ...
    init_virtual_unenv_global_polyfill_clear_immediate();
    init_cloudflare6();
    hashPassword2 = /* @__PURE__ */ __name((password, salt, iterations = 1e5, keyLength = 64) => {
      return new Promise((resolve, reject) => {
        pbkdf22(password, salt, iterations, keyLength, "sha256", (err, derivedKey) => {
          if (err) {
            return reject(err);
...
async function onRequestGet3(context2) {
  ...
  try {
    const payload = await decodeJWTToken(context2, token);
    if (!payload) {
    ...
    const responseData = {
      message: "Unauthorized: " + (error3 instanceof jwt3.TokenExpiredError ? "Token has expired" : "Invalid token")
    };
    return new Response(JSON.stringify(responseData), {
      headers: {
        "Content-Type": "application/json"
...
// api/checkin.ts
var onRequest, handlePost, handleGet;
var init_checkin = __esm({
  "api/checkin.ts"() {
    ...
    onRequest = /* @__PURE__ */ __name(async (context2) => {
      ...
      console.log(method);
      if (method === "POST") {
        return await handlePost(context2);
      } else if (method === "GET") {
        return await handleGet(context2);
      } else {
        return new Response("Method Not Allowed", { status: 405 });
      }
    }, "onRequest");
    handlePost = /* @__PURE__ */ __name(async (context2) => {
      const { request, env: env3 } = context2;
      console.log(`Request received: ${request.url}`);
      const token = await getJWTToken(context2);
      let JSONResponse = await responseTokenExists(token);
      ...
      try {
        ...
        const userIdQuery = await env3.DB.prepare(`SELECT id FROM user WHERE username = ?`).bind(username).first();
        const userId = userIdQuery.id;
        console.log("The user ID: " + userId);
        if (!date_worked) {
          return new Response(JSON.stringify({ error: "Date worked is required." }), { status: 400 });
        }
        const existingStreakQuery = await env3.DB.prepare(`
        ...
        const latestDateQuery = await env3.DB.prepare(`
        ...
        const insertResult = await env3.DB.prepare(`
    ...
    handleGet = /* @__PURE__ */ __name(async (context2) => {
      const { request, env: env3 } = context2;
      console.log(`Request received: ${request.url}`);
      const token = await getJWTToken(context2);
      let JSONResponse = await responseTokenExists(token);
...
// ../.wrangler/tmp/bundle-h2nHVl/middleware-loader.entry.ts
init_functionsRoutes_0_3572846533578271();
init_checked_fetch();
init_virtual_unenv_global_polyfill_process();
init_virtual_unenv_global_polyfill_performance();
...
// ../.wrangler/tmp/bundle-h2nHVl/middleware-insertion-facade.js
init_functionsRoutes_0_3572846533578271();
init_checked_fetch();
init_virtual_unenv_global_polyfill_process();
init_virtual_unenv_global_polyfill_performance();
...
*/
//# sourceMappingURL=functionsWorker-0.6727008856185435.mjs.map
